...
```java
   ...
   final Node<K,V>[] resize() {
        ...
        int oldThr = threshold; //容量=capacity * load factor
        int newCap, newThr = 0;
        //如果旧的table不为空
        if (oldCap > 0) {
            //判断旧表的容量是否超过容量的最大值
            ...
            //如果旧的table容量的两倍小于最大阈值，则newThr等于两倍旧的table容量
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY)
                newThr = oldThr << 1; // double threshold
        }
        //如果旧的阈值已经被初始化过了 那就直接将已经初始化过的oldThr赋值给newCap
        else if (oldThr > 0) // initial capacity was placed in threshold
            newCap = oldThr;
        else {               // zero initial threshold signifies using defaults
            //如果旧的table容量和阈值都没有被初始化过，则初始化新的容量和阈值
...
ref:https://zhuanlan.zhihu.com/p/34280652
```
